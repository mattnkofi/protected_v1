    ...
    <div class="sidebar-container">
        ...
        <aside :class="[
            ...
            <div class="h-full flex flex-col relative z-10">
                ...
                <!-- Logo / Brand -->
                <div
                    :class="['flex items-center h-24 relative transition-all duration-500 border-b border-platinum-200 dark:border-abyss-600', isFull ? 'px-6' : 'justify-center px-0']">
                    <div class="flex items-center gap-3 group/logo">
                        <div class="relative flex-shrink-0 transition-transform duration-500"
                            :class="isIcon && 'scale-90'">
                            <div
                                class="relative w-12 h-12 bg-gradient-to-br from-calm-lavender-600 via-neon-pink-500 to-calm-lavender-700 rounded-2xl flex items-center justify-center group-hover/logo:rotate-6 transition-all">
                        ...
                        <transition name="fade">
                            <div v-if="isFull" class="flex flex-col">
                                <h1
                                    class="text-xl font-black tracking-tighter text-abyss-900 dark:text-platinum-50 leading-none uppercase normal">
                                    Protect<span
                                        class="bg-gradient-to-r from-calm-lavender-600 to-neon-pink-500 bg-clip-text text-transparent">Ed</span>
                                </h1>
                                <span
                                    class="text-[9px] font-black text-abyss-600 dark:text-platinum-400 uppercase tracking-[0.3em] mt-1 normal leading-none">
                                    Academy Hub
                                </span>
                            </div>
                        </transition>
                ...
                <div
                    :class="['flex items-center py-6 relative', isFull ? 'justify-between px-8' : 'justify-center px-0']">
                    <transition name="fade">
                        <span v-if="isFull"
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-abyss-700 dark:text-platinum-400 normal">
                            Main Menu
                        </span>
                    </transition>
                    <button @click="sidebarStore.toggleExpanded()"
                        ...
                        <ChevronLeft v-if="isFull"
                            class="h-4 w-4 text-abyss-800 dark:text-platinum-200 group-hover:-translate-x-0.5 transition-transform" />
                        <ChevronRight v-else
                            class="h-4 w-4 text-abyss-800 dark:text-platinum-200 group-hover:translate-x-0.5 transition-transform" />
                    </button>
                </div>
                ...
                <nav class="flex-1 scroll-left-container custom-scrollbar relative py-2">
                    <div class="scroll-content space-y-1.5 pb-4 mt-2">
                        ...
                        <div v-for="(item, index) in navItems" :key="index">
                            <button @click="item.action ? item.action() : router.push({ name: item.route })" :class="[
                                ...
                                (item.isActive ? item.isActive() : route.name === item.route)
                                    ? 'container-bg-dark cursor-default'
                                    : 'bg-transparent hover:bg-platinum-200/50 dark:hover:bg-abyss-600/50 hover:translate-x-1'
                            ]">

                                <div v-if="(item.isActive ? item.isActive() : route.name === item.route)"
                                    class="absolute bottom-full right-0 w-6 h-6 pointer-events-none z-10
                                            ...
                                            dark:bg-[radial-gradient(circle_at_0_0,transparent_24px,var(--color-abyss-800)_24px)]">
                                </div>
                                ...
                                <div v-if="(item.isActive ? item.isActive() : route.name === item.route)"
                                    class="absolute top-full right-0 w-6 h-6 pointer-events-none z-10
                                            ...
                                            dark:bg-[radial-gradient(circle_at_0_100%,transparent_24px,var(--color-abyss-800)_24px)]">
                                </div>

                                <div
                                    :class="['flex items-center gap-4 w-full h-full relative z-20', isIcon ? 'justify-center pr-2' : 'pl-3']">

                                    <div class="w-10 h-10 rounded-[1.1rem] flex items-center justify-center transition-all duration-300 flex-shrink-0 logo-section"
                                        :class="(item.isActive ? item.isActive() : route.name === item.route)
                                        ...
                                        <component :is="item.icon"
                                            class="w-5 h-5 stroke-[2.5] transition-colors duration-300" :class="(item.isActive ? item.isActive() : route.name === item.route)
                                    ...
                                    <div v-if="!isIcon" class="flex flex-col items-start pt-1">
                                        <span class="transition-colors duration-300 inline-block relative pb-1"
                                            :class="(item.isActive ? item.isActive() : route.name === item.route) ? 'sidebar-active-item-name sidebar-item-name' : 'sidebar-inactive-item-name sidebar-item-name sidebar-inactive-item-name-hover'">
                                            {{ item.name }}

                                            <span
                                                class="absolute bottom-0 left-0 h-[2.5px] rounded-full transition-all duration-300 bg-gradient-to-r from-calm-lavender-600 to-neon-pink-500"
                                                :class="(item.isActive ? item.isActive() : route.name === item.route) ? 'w-full opacity-100' : 'w-0 opacity-0'">
                                            </span>
                                        </span>
                                    </div>
                                </div>
                ...
                <div class="mt-auto p-4 relative border-t border-platinum-200 dark:border-abyss-600 container-bg">
                    <div ref="profileTriggerRef" @click="profileMenuOpen = !profileMenuOpen"
                        ...
                        <div class="flex items-center gap-3 min-w-0" v-if="!isIcon">
                            ...
                            <div v-else
                                class="w-10 h-10 rounded-full bg-gradient-to-tr from-calm-lavender-600 via-neon-pink-500 to-calm-lavender-700 flex items-center justify-center text-[11px] font-black text-white shadow-md uppercase border border-white/20 normal">
                                {{ initials }}
                            </div>
                            <div class="flex-1 min-w-0 text-left">
                                <p
                                    class="text-[12px] font-black text-abyss-900 dark:text-platinum-50 truncate uppercase tracking-tighter leading-none normal">
                                    {{ auth.user?.name || 'Student' }}
                                </p>
                                <div class="flex items-center gap-2 mt-1">
                                    <p
                                        class="text-[9px] font-black text-calm-lavender-600 dark:text-calm-lavender-400 uppercase tracking-widest normal leading-none">
                        ...
                        <div
                            class="text-abyss-800 dark:text-platinum-300 hover:text-calm-lavender-600 transition-colors p-1 pr-2">
                            <MoreVertical v-if="!isIcon" class="h-4 w-4" />
                            <img v-else-if="userAvatar" :src="userAvatar" :alt="auth.user?.name"
                                class="w-10 h-10 rounded-full object-cover shadow-md" />
                            <div v-else
                                class="w-10 h-10 rounded-full bg-gradient-to-br from-neon-pink-500 to-calm-lavender-600 flex items-center justify-center text-[10px] font-black text-white uppercase shadow-md normal">
                                {{ initials }}
                            </div>
                        </div>
                    </div>
...
// On desktop: icon (collapsed) or full (expanded) driven by store.isExpanded
// On mobile: hidden or mobile-full driven by store.isMobileOpen
const mode = computed(() => {
    if (width.value < 1024) {
        return sidebarStore.isMobileOpen ? 'mobile-full' : 'hidden'
    }
...
.custom-scrollbar::-webkit-scrollbar-thumb {
    @apply bg-platinum-300 dark:bg-abyss-600 rounded-full;
}
