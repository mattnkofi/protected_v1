        ...
        <div class="modal-shell animate-modal">
            ...
            <form @submit.prevent="handleSubmit" class="modal-body custom-scrollbar space-y-5">
                ...
                <div class="form-section">
                    ...
                    <div class="field-group">
                        ...
                        <div class="custom-select-wrap">
                            ...
                            <Teleport to="#app">
                                <div v-show="openDropdown === 'classroom'" class="custom-select-panel-teleport" :style="panelStyles['classroom']">
                                    <button type="button"
                    ...
                    <!-- Module Title -->
                    <div class="field-group">
                        <label class="field-label">Module Title <span class="text-red-400">*</span></label>
                        <input
                            v-model="form.title"
                            type="text"
                            required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="custom-select-wrap">
                                ...
                                >
                                    <span class="font-mplusrounded text-base">
                                        {{ { gad: 'Institutional', sexual_health: 'Health', vawc: 'Safety', general: 'Standard' }[form.category] || 'Select category' }}
                                    </span>
                                    <ChevronDownIcon class="custom-select-chevron" :class="{ 'rotate-180': openDropdown === 'category' }" />
                                </button>
                                <Teleport to="#app">
                                    <div v-show="openDropdown === 'category'" class="custom-select-panel-teleport" :style="panelStyles['category']">
                                        <button type="button"
                        ...
                        <div class="field-group">
                            ...
                            <div class="custom-select-wrap">
                                ...
                                >
                                    <span class="font-mplusrounded text-base">
                                        {{ { beginner: 'Beginner', intermediate: 'Intermediate', advanced: 'Advanced' }[form.difficulty_level] || 'Select level' }}
                                    </span>
                                    <ChevronDownIcon class="custom-select-chevron" :class="{ 'rotate-180': openDropdown === 'difficulty' }" />
                                </button>
                                <Teleport to="#app">
                                    <div v-show="openDropdown === 'difficulty'" class="custom-select-panel-teleport" :style="panelStyles['difficulty']">
                                        <button type="button"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="upload-tile">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <span class="font-dosis text-sm font-semibold
                                             text-abyss-800 dark:text-platinum-100">Module File</span>
                            </div>
                            <label class="upload-zone cursor-pointer">
                                ...
                                <span class="font-mplusrounded text-sm font-normal
                                             text-platinum-600 dark:text-platinum-400 truncate">
                        ...
                        <div class="upload-tile">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <span class="font-dosis text-sm font-semibold
                                             text-abyss-800 dark:text-platinum-100">Cover Thumbnail</span>
                            </div>
                            <label class="upload-zone cursor-pointer">
                                <input
                                    type="file"
                ...
                <div class="form-section">
                    ...
                    <div class="toggle-row">
                        <label class="toggle-item group">
                            <input v-model="form.is_published" type="checkbox" class="toggle-checkbox" />
                            <div class="toggle-track">
                                <div class="toggle-thumb"></div>
                            </div>
                            <div>
                                <p class="font-dosis text-sm font-semibold text-abyss-800 dark:text-platinum-100
                        ...
                        <label class="toggle-item group">
                            <input v-model="form.is_featured" type="checkbox" class="toggle-checkbox" />
                            <div class="toggle-track">
                                <div class="toggle-thumb"></div>
                            </div>
                            <div>
                ...
                <div v-if="error" class="error-banner">
                    ...
                    <p class="font-mplusrounded text-sm font-medium leading-relaxed
                               text-red-600 dark:text-red-400">{{ error }}</p>
                </div>
            ...
            <footer class="modal-footer">
                ...
                >
                    <span v-if="!creating" class="font-dosis font-semibold text-sm uppercase tracking-wider">
                        Create Module
                    </span>
...
const handleFileSelect = (event) => {
    ...
    if (!allowedTypes.includes(file.type)) { alert('PDF or Word docs only.'); event.target.value = ''; return; }
    if (file.size > 25 * 1024 * 1024)     { alert('Limit: 25MB.');           event.target.value = ''; return; }
...
const handleThumbnailSelect = (event) => {
    const file = event.target.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/'))   { alert('Images only.');  event.target.value = ''; return; }
    if (file.size > 5 * 1024 * 1024)      { alert('Limit: 5MB.');   event.target.value = ''; return; }
...
.form-section-label {
    @apply font-dosis block text-xs font-bold uppercase tracking-widest mb-1;
    @apply text-platinum-600 dark:text-abyss-300;
}
...
.toggle-item     { @apply flex items-center gap-3 cursor-pointer flex-1; }
...
.divider-v { @apply w-px self-stretch bg-platinum-300 dark:bg-abyss-600; }
...
/* ── 3D buttons ───────────────────────────────────────────── */
.btn-3d           { @apply border-b-4 border-black/15 active:border-b active:translate-y-px; }
.btn-3d--secondary { @apply border-b-4 border-platinum-400 dark:border-abyss-500 active:border-b active:translate-y-px; }

/* ── Modal animation ──────────────────────────────────────── */
.animate-modal { animation: modalEntry 0.28s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
...
/* ── Custom dropdown ──────────────────────────────────────── */
.custom-select-wrap { @apply relative z-50; }
...
.custom-select-chevron {
    @apply w-4 h-4 text-platinum-500 dark:text-abyss-400 shrink-0 transition-transform duration-200;
}
.custom-select-option {
    @apply w-full text-left px-4 py-3 cursor-pointer transition-all duration-100;
    @apply font-mplusrounded text-base font-normal;
    @apply text-abyss-800 dark:text-platinum-200;
...
.custom-select-option-active {
    ...
    @apply text-calm-lavender-700 dark:text-calm-lavender-300 font-medium;
}
</style>
...
.custom-select-panel-teleport .custom-select-option:hover {
    background-color: var(--color-calm-lavender-50);
    color: var(--color-calm-lavender-700);
}
.dark .custom-select-panel-teleport .custom-select-option:hover {
    background-color: var(--color-abyss-800);
    color: var(--color-calm-lavender-300);
}
.custom-select-panel-teleport .custom-select-option-active {
    background-color: var(--color-calm-lavender-100);
    color: var(--color-calm-lavender-700);
    font-weight: 600;
}
.dark .custom-select-panel-teleport .custom-select-option-active {
    background-color: color-mix(in srgb, var(--color-calm-lavender-900) 35%, var(--color-abyss-950));
    color: var(--color-calm-lavender-300);
    font-weight: 600;
}
</style>