<template>
    <div class="min-h-screen bg-slate-950 text-slate-100 font-['Dosis'] relative overflow-hidden">
        <div class="pointer-events-none absolute inset-0">
            <div class="absolute -top-32 left-0 h-96 w-96 rounded-full bg-cyan-500/10 blur-3xl"></div>
            <div class="absolute top-16 right-0 h-[28rem] w-[28rem] rounded-full bg-amber-500/10 blur-3xl"></div>
        </div>
        ...
        <div class="relative px-3 py-6 lg:px-8 space-y-8">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-xs uppercase tracking-[0.4em] text-slate-400">WGAD Workspace</p>
                    <h1 class="mt-2 text-3xl sm:text-4xl font-semibold text-slate-50">Manage Proposals</h1>
                    <p class="mt-2 text-sm text-slate-300">Submit, upload documents, and track progress in one place.</p>
                </div>
                <div class="rounded-full border border-slate-700/70 bg-slate-900/70 px-4 py-2 text-xs text-slate-300">
                    {{ proposals.length }} proposals
                </div>
            </div>

            <div v-if="successMessage" class="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm font-semibold text-emerald-200">
                <div class="flex items-center gap-3">
                    <CheckCircle class="w-5 h-5 text-emerald-300 shrink-0" />
                    <p>{{ successMessage }}</p>
                </div>
            </div>
            <div v-if="errorMessage" class="rounded-2xl border border-rose-500/40 bg-rose-500/10 p-4 text-sm font-semibold text-rose-200">
                <div class="flex items-center gap-3">
                    <div class="w-5 h-5 text-rose-300 shrink-0">!</div>
                    <p>{{ errorMessage }}</p>
                </div>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-[1.05fr,1.5fr] gap-8">
                <div class="space-y-6">
                    <div class="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-6 shadow-lg">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="flex h-8 w-8 items-center justify-center rounded-full border border-cyan-400/40 bg-cyan-500/10 text-xs font-semibold text-cyan-200">01</div>
                                <h2 class="text-xs uppercase tracking-[0.3em] text-slate-300">New Proposal</h2>
                            </div>
                        </div>

                        <div class="mt-5 space-y-4">
                            <div>
                                <label class="block text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Campus</label>
                                <select v-model="newProposal.campus_id" class="w-full rounded-xl border border-slate-700/70 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/70">
                            ...
                            <div>
                                <label class="block text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Title</label>
                                <input v-model="newProposal.title" type="text" placeholder="Enter proposal title..." class="w-full rounded-xl border border-slate-700/70 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/70" />
                            </div>

                            <div>
                                <label class="block text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Description</label>
                                <textarea v-model="newProposal.description" rows="4" placeholder="Describe your gender-responsive initiative..." class="w-full rounded-xl border border-slate-700/70 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/70 resize-none" />
                                ...
                                <div v-if="newProposal.description" class="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <p class="text-xs font-semibold text-amber-200 uppercase tracking-[0.2em]">Language Check</p>
                                    </div>
                                    <div v-if="previewAnalysis.issues.length > 0" class="text-xs space-y-1">
                                        <div v-for="issue in previewAnalysis.issues.slice(0, 2)" :key="`${issue.term}-${issue.severity}`" class="flex items-center gap-2">
                                            <span class="text-amber-200">"{{ issue.term }}"</span>
                                            <span :class="['px-2 py-0.5 rounded text-[10px] font-semibold uppercase', issue.severity === 'high' ? 'bg-rose-500/30 text-rose-200' : 'bg-yellow-500/30 text-yellow-200']">
                                                {{ issue.severity }}
                                            </span>
                                        </div>
                                        <p v-if="previewAnalysis.issues.length > 2" class="text-slate-400 text-xs">+{{ previewAnalysis.issues.length - 2 }} more</p>
                                    </div>
                                    <div v-else class="flex items-center gap-2 text-emerald-300 text-xs font-semibold">
                            ...
                            <button
                                ...
                                class="w-full rounded-2xl bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 px-6 py-3 text-sm font-semibold text-slate-950 transition hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="flex h-8 w-8 items-center justify-center rounded-full border border-amber-400/40 bg-amber-500/10 text-xs font-semibold text-amber-200">02</div>
                            <h2 class="text-xs uppercase tracking-[0.3em] text-slate-300">Your Proposals</h2>
                        </div>
                    </div>

                    <div v-if="proposals.length === 0" class="rounded-2xl border border-dashed border-slate-800/70 bg-slate-900/50 p-10 text-center">
                        <div class="w-16 h-16 bg-cyan-500/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <FileIcon class="w-8 h-8 text-cyan-300" />
                        </div>
                        <p class="text-slate-300 font-semibold">No proposals yet</p>
                        <p class="text-xs text-slate-500">Submit your first proposal to get started.</p>
                    </div>

                    <div v-for="proposal in proposals" :key="proposal.id" class="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-6 space-y-4 transition hover:border-cyan-400/60">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <h3 class="text-base font-semibold text-slate-100 mb-1 line-clamp-2">{{ proposal.title }}</h3>
                                <p class="text-xs text-slate-400">{{ proposal.campus?.name }} - {{ formatDate(proposal.submission_date) }}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 text-xs border-t border-slate-800/70 pt-4">
                            <div class="rounded-xl border border-slate-800/70 bg-slate-950/60 p-3">
                                <p class="text-slate-400 text-xs uppercase tracking-[0.2em]">Campus</p>
                                <p class="text-slate-100 font-semibold truncate">{{ proposal.campus?.name || '-' }}</p>
                            </div>
                            <div class="rounded-xl border border-slate-800/70 bg-slate-950/60 p-3">
                                <p class="text-slate-400 text-xs uppercase tracking-[0.2em]">Language</p>
                                <p class="text-slate-100 font-semibold">{{ proposal.gfl_score || '-' }}%</p>
                            </div>
                            <div class="rounded-xl border border-slate-800/70 bg-slate-950/60 p-3">
                                <p class="text-slate-400 text-xs uppercase tracking-[0.2em]">Document</p>
                                <p :class="['font-semibold', proposal.file_key ? 'text-emerald-300' : 'text-slate-400']">
                        ...
                        <div v-if="proposal.total_score !== undefined" class="rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-semibold text-cyan-200 uppercase tracking-[0.2em]">Score</p>
                                    <p class="text-2xl font-semibold text-cyan-100">{{ proposal.total_score }}/{{ proposal.max_score }}</p>
                                </div>
                                <div class="text-right">
                                    <div class="w-16 h-16 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-slate-950 font-semibold">
                        ...
                        <div v-if="proposal.admin_feedback" class="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
                            <p class="text-xs font-semibold text-amber-200 uppercase tracking-[0.2em] mb-2">Admin Comments</p>
                            <p class="text-sm text-slate-200 leading-relaxed">{{ proposal.admin_feedback }}</p>
                        </div>

                        <div class="border-t border-slate-800/70 pt-4 space-y-3">
                            <div class="flex items-center justify-between">
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Supporting Document</p>
                                <span v-if="proposal.file_key" class="flex items-center gap-1 text-xs font-semibold text-emerald-300">
                            ...
                            <div v-if="proposal.file_key" class="p-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <FileIcon class="w-4 h-4 text-emerald-300" />
                                    <div>
                                        <p class="text-xs text-emerald-100 font-semibold">{{ proposal.file_key.split('/').pop() }}</p>
                                        <p class="text-xs text-slate-400">{{ formatDate(proposal.updated_at) }}</p>
                                    </div>
                                </div>
                                <button
                                    @click="openDocumentViewer(proposal)"
                                    class="rounded-full border border-emerald-500/40 px-3 py-1 text-xs font-semibold text-emerald-200 transition hover:border-emerald-400/70"
                                >
                                    View
                                </button>
                            </div>
                            ...
                            <div v-if="!proposal.file_key" class="space-y-2">
                                <div
                                    ...
                                    :class="activeDragProposal === proposal.id ? 'border-cyan-400/80 bg-cyan-500/10' : 'border-slate-700/80 bg-slate-950/60 hover:border-slate-500/80'"
                                >
                                    <FileIcon class="w-5 h-5 mx-auto mb-1 text-cyan-300" />
                                    <p class="text-xs font-semibold mb-1">Drop or click to upload</p>
                                    <p class="text-xs text-slate-400">PDF/DOCX, max 50MB</p>
                                    <input
                                        type="file"
                                ...
                                <div v-if="pendingFiles[proposal.id]" class="p-3 rounded-xl border border-cyan-500/30 bg-cyan-500/10 flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <FileIcon class="w-4 h-4 text-cyan-300" />
                                        <div class="text-xs">
                                            <p class="font-semibold text-slate-100">{{ pendingFiles[proposal.id].name }}</p>
                                            <p class="text-slate-400">{{ formatFileSize(pendingFiles[proposal.id].size) }}</p>
                                        </div>
                                    </div>
                                    <button
                                ...
                                <div class="flex gap-2">
                                    <button
                                        ...
                                        class="flex-1 rounded-xl border border-slate-700/70 bg-slate-950/60 px-3 py-2 text-xs font-semibold text-slate-200 transition hover:border-cyan-400/60"
                                    >
                                        Choose File
                                    </button>
                                    <button
                                        ...
                                        class="flex-1 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-3 py-2 text-xs font-semibold text-slate-950 transition hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
