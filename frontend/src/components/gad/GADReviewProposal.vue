    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 font-['Dosis'] relative overflow-hidden">
        ...
        <div class="relative px-3 py-6 lg:px-8 space-y-8">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-xs uppercase tracking-[0.4em] text-slate-400">WGAD Review</p>
                    <h1 class="mt-2 text-3xl sm:text-4xl font-semibold text-slate-50">Review Proposal</h1>
                    <p class="mt-2 text-sm text-slate-300">Provide feedback and score the proposal.</p>
                </div>
            </div>

            <div v-if="proposal" class="space-y-8">
                <div class="rounded-3xl border border-slate-800/70 bg-slate-900/70 p-8 space-y-6 shadow-lg">
                    <div>
                        <h2 class="text-2xl font-semibold text-slate-50">{{ proposal.title }}</h2>
                        <div class="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-slate-300 bg-slate-950/60 p-4 rounded-2xl border border-slate-800/70">
                            <div>
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Submitted by</p>
                                <p class="font-semibold text-slate-100">{{ proposal.user?.name }}</p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Campus</p>
                                <p class="font-semibold text-slate-100">{{ proposal.campus?.name }}</p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Date</p>
                                <p class="font-semibold text-slate-100">{{ formatDate(proposal.submission_date) }}</p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Status</p>
                    ...
                    <div>
                        <h3 class="text-xs uppercase tracking-[0.3em] text-slate-400 mb-3">Description</h3>
                        <p class="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/70 text-slate-100 leading-relaxed">{{ proposal.description }}</p>
                    </div>
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h3 class="text-sm font-semibold text-cyan-200 uppercase tracking-[0.2em]">Supporting Document</h3>
                                </div>
                                <p class="text-xs text-cyan-200 font-mono">{{ proposal.file_key.split('/').pop() }}</p>
                                <p class="text-xs text-slate-400 mt-1">Uploaded: {{ formatDate(proposal.updated_at) }}</p>
                            </div>
                            <button
                                @click="showDocumentViewer = true"
                                class="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-slate-950 text-xs font-semibold rounded-xl transition-colors whitespace-nowrap"
                            >
                                <Eye class="w-4 h-4" />
                                View Document
                            </button>
                    ...
                    <div v-if="proposal.gfl_issues?.issues?.length > 0" class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <h3 class="text-sm font-semibold text-amber-200 uppercase tracking-[0.2em]">Gender-Fair Language Issues Detected</h3>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(issue, idx) in proposal.gfl_issues.issues" :key="idx" class="text-sm">
                                <strong class="text-amber-200">"{{ issue.term }}"</strong>
                                ...
                                -> <em class="text-emerald-300">"{{ issue.suggestion }}"</em>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="rounded-3xl border border-slate-800/70 bg-slate-900/70 p-8 space-y-6">
                    <h2 class="text-2xl font-semibold">Evaluation Criteria</h2>

                    <div v-if="criteria.length === 0" class="text-center py-8 text-slate-500">
                        <p class="font-semibold">Loading criteria...</p>
                    </div>

                    <div v-for="crit in criteria" :key="crit.id" class="p-4 bg-slate-950/60 border border-slate-800/70 rounded-2xl">
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex-1">
                                <h3 class="font-semibold uppercase tracking-[0.2em] text-slate-200">{{ crit.criteria_name }}</h3>
                                <p class="text-sm text-slate-400 mt-1">{{ crit.description }}</p>
                            </div>
                        ...
                        <div class="space-y-3">
                            <div>
                                <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400 mb-2 block">Points Awarded</label>
                                <div class="flex items-center gap-4">
                                    <input
                                        ...
                                        min="0"
                                        step="1"
                                        class="flex-1 h-2 bg-slate-950/80 rounded-full cursor-pointer"
                                    />
                            ...
                            <div>
                                <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400 mb-2 block">Evaluator Comment</label>
                                <textarea
                                    v-model="comments[crit.id]"
                                    rows="2"
                                    placeholder="Provide specific feedback for this criterion (visible to facilitator)..."
                                    class="w-full px-3 py-2 bg-slate-950/70 text-slate-100 border border-slate-800/70 rounded-xl placeholder-slate-500 focus:border-cyan-400 focus:outline-none transition-all text-sm resize-none"
                                />
                ...
                <div class="rounded-3xl border border-slate-800/70 bg-slate-900/70 p-8 space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-semibold uppercase tracking-[0.2em] mb-3">Decision</label>
                        <select
                            v-model="reviewStatus"
                            class="w-full px-4 py-3 bg-slate-950/70 text-slate-100 border border-slate-800/70 rounded-xl focus:border-cyan-400 focus:outline-none transition-all"
                        >
                    ...
                    <div>
                        <label class="block text-sm font-semibold uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                        ...
                        <textarea
                            ...
                            class="w-full px-4 py-3 bg-slate-950/70 text-slate-100 border border-slate-800/70 rounded-xl placeholder-slate-500 focus:border-cyan-400 focus:outline-none transition-all resize-none"
                        />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-slate-900/70 border border-slate-800/70 rounded-2xl p-5">
                        <p class="text-xs text-slate-500 uppercase tracking-[0.2em] mb-2">Total Score</p>
                        <p class="text-3xl font-semibold text-cyan-300">{{ calculateTotalScore }}</p>
                    </div>
                    <div class="bg-slate-900/70 border border-slate-800/70 rounded-2xl p-5">
                        <p class="text-xs text-slate-500 uppercase tracking-[0.2em] mb-2">Max Possible</p>
                        <p class="text-3xl font-semibold text-slate-300">{{ calculateMaxScore }}</p>
                    </div>
                    <div class="bg-slate-900/70 border border-slate-800/70 rounded-2xl p-5">
                        <p class="text-xs text-slate-500 uppercase tracking-[0.2em] mb-2">Percentage</p>
                        <p class="text-3xl font-semibold text-amber-300">{{ calculatePercentage }}%</p>
                    </div>
                    <div class="bg-slate-900/70 border border-slate-800/70 rounded-2xl p-5">
                        <p class="text-xs text-slate-500 uppercase tracking-[0.2em] mb-2">Decision</p>
                ...
                <div class="flex gap-4">
                    <button
                        ...
                        class="flex items-center justify-center gap-2 flex-1 px-6 py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-semibold uppercase tracking-[0.2em] rounded-2xl hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                    >
                        <Check class="w-5 h-5" />
                    ...
                    <button
                        @click="$router.back()"
                        class="px-6 py-4 border border-slate-700/70 bg-slate-900/70 text-slate-100 font-semibold uppercase tracking-[0.2em] rounded-2xl hover:border-slate-500 transition-all"
                    >
                        Cancel
                    </button>
                </div>
                ...
                <div v-if="successMessage" class="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                    <p class="text-emerald-200 font-semibold">{{ successMessage }}</p>
                </div>
            </div>
            ...
            >
                <div class="bg-slate-950 rounded-2xl border border-slate-800/70 w-full max-w-5xl max-h-[90vh] flex flex-col">
                    <div class="flex justify-between items-center p-6 border-b border-slate-800/70">
                        <div>
                            <h3 class="text-lg font-semibold text-slate-100 uppercase tracking-[0.2em]">Document Viewer</h3>
                            <p class="text-xs text-slate-400">{{ proposal?.file_key?.split('/').pop() }}</p>
                        </div>
                        <button
...
const getStatusColor = (status) => {
    const colors = {
        ...
        rejected_with_feedback: 'text-rose-300',
        revised: 'text-orange-300',
        approved: 'text-emerald-300',
        approved_final: 'text-teal-300'
    };
...
<template>
    <div class="min-h-screen bg-slate-950 text-slate-100 font-['Dosis'] relative overflow-hidden">
        ...
        <div class="relative mx-auto max-w-6xl px-6 py-10 lg:px-10 space-y-8">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-xs uppercase tracking-[0.4em] text-slate-400">WGAD Review</p>
                    <h1 class="mt-2 text-3xl sm:text-4xl font-semibold text-slate-50">Review Proposal</h1>
