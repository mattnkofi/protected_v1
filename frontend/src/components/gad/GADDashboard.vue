<template>
    <div class="page-wrapper animate-in">
        <header class="page-header">
            <div class="space-y-2">
                <p class="section-eyebrow">WGAD Admin</p>
                <h1 class="page-title">WGAD <span class="brand-gradient-text">Dashboard</span></h1>
        ...
        <div class="card space-y-6">
            ...
            <div v-if="pendingProposals.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                    v-for="proposal in pendingProposals"
                    :key="proposal.id"
                    @click="selectProposal(proposal)"
                    class="item-card !p-4 cursor-pointer hover:border-amber-200 dark:hover:border-amber-700/60 h-full"
                >
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-platinum-400">Pending Proposal</p>
                            <h3 class="mt-1 font-semibold text-slate-800 dark:text-platinum-100 line-clamp-1">{{ proposal.title }}</h3>
                            <p class="mt-1 text-xs text-slate-500 dark:text-platinum-400">{{ proposal.user?.name }} • {{ formatDate(proposal.submission_date) }}</p>
                        </div>
                        <button
                    ...
                    <div class="mt-4 rounded-xl border border-slate-200/80 dark:border-abyss-500/80 bg-white dark:bg-abyss-800 p-4">
                        <p class="text-[10px] uppercase tracking-[0.2em] text-slate-500 dark:text-platinum-400">Summary</p>
                        <p class="mt-2 text-sm text-slate-600 dark:text-platinum-300 line-clamp-2">{{ proposal.description }}</p>
                    </div>
                </div>
            </div>
